...
#Task name: OverDueTasksNotionWhatsapp
# This script is run everyday in the night on 10:00.
...
def ReadDatabase(databaseId, headers):
    ...
    print(json_string)
    columns = ['Id', 'Name', 'Notes', 'Deadline', 'Type', 'CreatedAt', 'Status', 'TaskFor', 'DueDate']
    ...
    for expense in json_string['results']:
        ...
        row.append(expense['properties']['Duedate']['formula']['string'])
...
for row in Results.values:
    if row[8] == 'Overdue' and row[6] != 'Completed':
        if row[7]:
            for name in row[7]:
                if name != 'Maryam':
                    now = datetime.now()
                        ...
                        for type in row[4]:
                            if type == 'Appointment':
                                pywhatkit.sendwhatmsg(PhoneNumbers[name],
                                                      'Apnnay apni appointment miss kardie hai: ' + row[
                                                          1] + ' is time py thi' + str(row[3]) + ' - ' + row[2]
                                                      , hour, minute)
                            else:
                                pywhatkit.sendwhatmsg(PhoneNumbers[name],
                                                      'Apka yeh task time sy mukamal nahe hwa: ' + row[1] + ' - ' + row[2]
                                                      + ' App please issy jald as jald mukamal karryn!'
                                                      , hour, minute)
